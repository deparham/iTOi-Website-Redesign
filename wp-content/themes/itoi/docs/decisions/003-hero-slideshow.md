...
The Live Detection bounding-box visualization described above —
`initHeroDetectionBoxes()` — removed from the hero entirely, explicit
instruction. The unrelated background-video static-media guard that lived
inside that same function survives under its own name,
`initHeroStaticMediaGuard()` (`assets/js/homepage.js`). The slideshow
mechanism itself (this doc's actual subject) is untouched.
