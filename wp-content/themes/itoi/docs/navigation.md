...
## Update 2026-08-31 — "Products" temporarily hidden

Per instruction, the "Products" top-level item is pulled from the primary
nav **for now**, code-side only. `header.php`'s `$itoi_mega_items` builder
skips the item whose destination is `/products/` (or any item carrying a
`nav-keep-hidden` CSS class), so it drops out of both the desktop bar and
the mobile mega-menu. The item is untouched in `Appearance → Menus` —
reverting is removing that skip block. With the bar down to 4 items
(Solutions · Industries · Resources · Company) the desktop `<nav>` is now
`justify-center` with `gap-1` (was left-packed, `gap-0.5`) so it doesn't
leave a wide dead gap before the right-hand CTAs.
