...
(function () {
	...
	function initFinder() {
		...
		// 2026-08-05: the floating trigger and front-page.php's Final CTA
		// band (#finalCtaBand) carry the exact same "Build your solution"
		// wording — with the trigger fixed at bottom-right, scrolling that
		// band into view stacked two identical CTAs on top of each other in
		// the same corner. triggerUnlocked tracks the existing "has this
		// been auto-shown/clicked yet" state (previously just a class add);
		// nearDuplicateCta additionally hides it while a same-wording CTA is
		// on screen, restoring it once scrolled away. Homepage-only in
		// practice — #finalCtaBand doesn't exist on other templates, so this
		// no-ops safely everywhere else.
		var triggerUnlocked = false;
		var nearDuplicateCta = false;
		function refreshTriggerVisibility() {
			trigger.classList.toggle( 'visible', triggerUnlocked && ! nearDuplicateCta );
		}
		var duplicateCta = document.getElementById( 'finalCtaBand' );
		if ( duplicateCta && 'IntersectionObserver' in window ) {
			var duplicateCtaObserver = new IntersectionObserver( function ( entries ) {
				entries.forEach( function ( entry ) {
					nearDuplicateCta = entry.isIntersecting;
					refreshTriggerVisibility();
				} );
			}, { threshold: 0.15 } );
			duplicateCtaObserver.observe( duplicateCta );
		}
		...
		function openModal() {
			...
			triggerUnlocked = true;
			refreshTriggerVisibility();
			document.body.style.overflow = 'hidden';
			// Same trap for both the auto-opened (3.5s timer, below) and
			// manually-clicked cases — closeBtn either way, not whichever
			// element happens to be focused at the moment this fires.
		...
		function closeModal() {
			...
			triggerUnlocked = true; // stays available to reopen
			refreshTriggerVisibility();
