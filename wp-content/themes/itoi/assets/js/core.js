...
(function () {
	...
	// ---------------------------------------------------------------
	// Generic stagger-reveal primitive — 2026-08-21 ("hero page-load
	// reveal" + "trust section scroll reveal"). Distinct from
	// initRevealObserver() above: that one is a single opt-in block
	// (.itoi-reveal) revealed as a unit; this staggers a LIST of sibling
	// items with a per-item delay, either immediately on page load or on
	// scroll-into-view of a shared trigger element — one portable utility
	// any section can call rather than each inventing its own timing
	// logic (per the explicit ask: "portable across the theme"). Exposed
	// on `window` (not just this closure) since the JS bundle split
	// (2026-08-06, see NOTES.md) put core.js and homepage.js in separate
	// IIFEs — homepage.js is the caller for both current use sites (the
	// hero and the trust section are both homepage-only), core.js just
	// owns the shared mechanism, same division as itoiCreateFocusTrap()
	// above serving initFinder()/initMegaMenu().
	//
	// items: NodeList/array of elements to reveal, each getting the
	//   .is-visible class added in order, with a computed transition-delay
	//   of `opts.baseDelay + index * opts.step` milliseconds — relies on
	//   each item already carrying the .itoi-stagger-item base class
	//   (src/tailwind.css) for its actual opacity/transform CSS.
	// opts.trigger: 'load' (default) fires immediately — for above-the-
	//   fold content already in view at page load (the hero). 'scroll'
	//   observes opts.root (or items[0] if omitted) via IntersectionObserver
	//   and fires once, the first time it enters the viewport — for
	//   content further down the page (the trust section).
	// opts.step: ms between each item's delay (default 90).
	// opts.baseDelay: ms added to every item's delay (default 0) — lets a
	//   later group in the same sequence (e.g. stat cards, after logos)
	//   start counting from where the previous group left off.
	// opts.threshold: IntersectionObserver threshold for 'scroll' (0.2).
	//
	// reduceMotion: every item's .is-visible is added immediately, no
	// delay/observer at all — same convention as initRevealObserver().
	// ---------------------------------------------------------------
	function itoiStaggerReveal( items, opts ) {
		items = Array.prototype.slice.call( items || [] ).filter( Boolean );
		if ( ! items.length ) {
			return;
		}
		opts = opts || {};
		var step = 'number' === typeof opts.step ? opts.step : 90;
		...
		if ( 'scroll' === opts.trigger ) {
			...
			).observe( opts.root || items[ 0 ] );
		} else {
			reveal();
		}
	}
	window.itoiStaggerReveal = itoiStaggerReveal;
