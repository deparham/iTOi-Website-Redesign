...
// ---------------------------------------------------------------------
add_filter(
	'manage_case_study_posts_columns',
	function ( $columns ) {
		return itoi_admin_columns_insert_after_title(
			$columns,
			array(
				'itoi_client'   => 'Client',
...
add_action(
	'manage_sb_lead_posts_custom_column',
	function ( $column, $post_id ) {
		$map = array(
...
add_filter(
	...
	function ( $query ) {
		...
		$query->set(
			'meta_query',
			array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- admin list filter, low-traffic screen.
			array(
