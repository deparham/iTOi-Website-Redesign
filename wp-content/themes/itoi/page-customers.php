<?php
/**
 * Customers page (slug `customers`, page ID 23524 — was a placeholder
 * "coming soon" page, now built out). `client` is a non-public CPT (same
 * pattern as team_member) queried directly here, filtered client-side by
 * the `client_category` taxonomy — the live portfolio page's own 6
...
while ( have_posts() ) :
	...
	<section class="border-b border-line bg-hero-bg px-8 pt-[168px] pb-16 min-[640px]:pt-[206px] min-[980px]:pb-[70px]">
		<div class="mx-auto max-w-[840px]">
			<div class="mb-2 text-[13.5px] font-bold uppercase tracking-wide text-teal-800">Customers</div>
	...
	<section class="px-8 py-16 min-[980px]:py-[70px]">
		<div class="mx-auto max-w-[1280px]">
			...
			<?php if ( ! is_wp_error( $itoi_client_terms ) && $itoi_client_terms ) : ?>
				<div class="mb-10 flex flex-wrap gap-2.5" id="clientFilterRow" role="group" aria-label="Filter customers by category">
			...
			<?php else : ?>
				<p class="text-text-muted">No customers published yet.</p>
			<?php endif; ?>
		</div>
	</section>
